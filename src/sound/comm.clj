  ...
  (when-let [merged-devices (arrange-device-data devices)]
    ...
    (let [vmap (vec merged-devices)
      ...
      (doall (mapv (fn [[id data]]
                     (when (last (:type data))
                       (case (last (:type data))
                         0.0 (do (pprint [id (last (:type data)) num vmap]) (axis-mapped-no-touch [id data] -250 250 :sample-blend :thunder-storm))
                         1.0 (do (pprint [id (last (:type data)) num vmap]) (axis-mapped-no-touch-upper-sensor [id data] -250 250 :sample-blend :thunder-storm))
                         2.0 (do (pprint [id (last (:type data)) num vmap]) (axis-mapped [id data] -250 250 :sample-blend :thunder-storm))
                         (pprint data)))) vmap)))
